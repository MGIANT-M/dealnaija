...
router.post('/register', async (req, res) => {
  try {
    ...
    const result = await db.query(
      'INSERT INTO users (email, password_hash, full_name, phone, role, referral_code, referred_by, agency_name, is_approved) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING id, email, full_name, role, referral_code, is_approved',
...
router.get('/me', auth, async (req, res) => {
  try {
    const result = await db.query(
      'SELECT id, email, full_name, phone, role, is_approved, avatar_url, referral_code, reputation_score, verified_listings_count, deals_closed_count, total_commission_earned, referral_count, agency_name, bio FROM users WHERE id = $1',
      [req.user.id]
    );
